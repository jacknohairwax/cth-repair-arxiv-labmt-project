...
# ``cat:cs.AI`` may end up with a different primary, e.g. ``cs.LG``. 
# The comparison is scoped to abstracts whose primary category is one
# of the six target categories. This keeps the aggregate comparison
# like-for-like rather than mixing in papers primarily filed elsewhere. 
# The README discusses this filtering decision explicitly in Section 9 (Critical reflection).
