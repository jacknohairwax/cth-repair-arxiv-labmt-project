...
> The numerical results below come from `tables/category_summary_table.csv` and `tables/robustness_check_table.csv`, generated by the pipeline.
> `tables/category_summary_table.csv` and
> `tables/robustness_check_table.csv` after running the pipeline. They
> are not invented. The committed CSVs are the canonical source.
